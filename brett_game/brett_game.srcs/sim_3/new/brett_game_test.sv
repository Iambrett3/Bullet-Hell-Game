...
module brett_game_test;
	...
	wire [`smem_Abits-1:0] smem_addr      =uut.smem_addr;             // address from vgadisplaydriver to access screen mem
	wire [`smem_Dbits-1:0]  charcode       =uut.charcode;              // character code returned by screen mem
	wire dmem_wr               =uut.memIO.dmem_wr;
	wire smem_wr               =uut.memIO.smem_wr;
	wire [31:0] mips_addr             =uut.memIO.mips_addr;
	wire [`smem_Dbits-1:0] smem_readdata         =uut.memIO.smem_readdata;
	wire [`smem_Dbits-1:0] mem_map_readdata = uut.memIO.memmap.readdata;
	wire [`smem_Dbits-1:0] smem_mem_map_out = uut.memIO.screenmem.memmap_readdata;
	wire [16:0] smem_internal_memmapaddr = uut.memIO.screenmem.memmapaddr;
